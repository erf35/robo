...
class Dobot:
    ...
    def get_ir(self, port=PORT_GP4):
        ...
        msg.params.extend(bytearray([port]))
        response = self._send_command(msg)
        if not response.params:
            raise DobotException("Empty response for IR read")

        # Some firmware variants return [port, state], others just [state].
        if len(response.params) >= 2 and response.params[0] == port:
            state_byte = response.params[1]
        else:
            state_byte = response.params[0]
